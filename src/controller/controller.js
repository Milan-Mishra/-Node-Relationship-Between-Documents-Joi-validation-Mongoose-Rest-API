...
exports.AddStudentToTeacher = async (req, res) => {
  ...
  const data = await teacher_data.findByIdAndUpdate(
    teacher_id,
    {
      $push: {
...
exports.GetStudents = async (req, res) => {
  ...
  data.length
    ? res.send({ message: "all student data", data })
    : res.send({ message: "No data available to display" });
};
...
exports.GetTeachers = async (req, res) => {
  try {
    const data = await teacher_data
      ...
      .populate("students", "name class role_no -_id");
    data.length
      ? res.send({ message: "all teachers data", data })
      : res.send({ message: "No data available to display" });
  } catch (err) {
    res.send({ message: "error in fetch data due to:" + err.message });
  }
...
exports.DeleteStudent = async (req, res) => {
  ...
  res.send({
    message: "student data of " + data.name + " sucessfully deleted",
    deleted_data: data,
  });
};
...
exports.DeleteTeacher = async (req, res) => {
  ...
  try {
    ...
    if (!data.students.length) {
      ...
      res.send({
        message: "Teacher data of " + data.name + " sucessfully deleted",
        deleted_data: data,
      });
    } else
      res.send({
        message: "Please remove the remaing student data first",
        student_data,
        status: "failed",
      });
  } catch (err) {
...
exports.DeleteAllData = async (req, res) => {
  ...
  const sucess_string =
    delete_students.deletedCount || delete_teachers.deletedCount
      ? `${
          delete_students.deletedCount + delete_teachers.deletedCount
        } Documemt(s) has been deleted!`
